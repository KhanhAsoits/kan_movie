...
const TicketScreen = () => {
    ...
    useEffect(() => {
        const bsSync = async () => {
            await TicketStore.onGetTicket()
            TicketStore.setNewTicket(0)
        }
        if (TicketStore.new_ticket > 0) {
            bsSync()
        }
    }, [TicketStore.new_ticket])
    ...
    const TicketBox = ({ticket}) => {
        ...
        return (
            <Box borderRadius={8} justifyContent={'center'} alignItems={'center'}
                 height={SCREEN_HEIGHT - bottomTabHeight - 80}>
                    ...
                    <VStack borderBottomRadius={8} bgColor={ThemeStore.baseProps.themeBg} px={4} pt={3} pb={5}
                        ...
                        <HStack justifyContent={'space-between'}
                            ...
                            <VStack width={'50%'}>
                                ...
                                <Text color={ThemeStore.baseProps.text_24} fontWeight={'500'}
                                      fontSize={17}>{ticket.selectedSeats.map((val, index) => {
                        ...
                        <Box borderRadius={8}>
                            <Barcode format="CODE128" value={numberTokenGenerator(14)}
                                     style={{width: '100%', height: 80}} lineColor={ThemeStore.baseProps.text_24}/>
