    ...
    const onGetTransactionResult = async (webViewState) => {
        if (webViewState.url.includes('https://kan_movie.com/')) {
            ...
            if (webViewState.url.includes('https://kan_movie.com/return')) {
                ...
                Alert.alert('Notification', 'You have cancel transaction.')
            }
        }
    }
                ...
                <NativeBaseProvider>
                    <Box flex={1} px={4} style={{height: 200, overflow: 'hidden'}}>
                        <ScrollView showsVerticalScrollIndicator={false}>
                                ...
                                <HStack space={3} bgColor={'white'} overflow={'hidden'} p={5} borderRadius={8}>
                                    ...
                                    <VStack>
                                        ...
                                        <Text width={SCREEN_WIDTH / 1.7} fontSize={16}
                                              fontWeight={'300'}
                                              color={'gray.400'}>{TicketStore.orderTicket.time?.hours} : {TicketStore.orderTicket.time?.minute} - {TicketStore.orderTicket.time?.hours + 2} : {TicketStore.orderTicket.time?.minute} {TicketStore.orderTicket.time.hours > 12 ? 'PM' : 'AM'}</Text>
                                        <Text width={SCREEN_WIDTH / 1.7} fontSize={16}
                            ...
                            {/*    list orders*/}
                            {TicketStore.orderTicket.extra.length > 0 &&
                                <Text fontSize={20} my={5}>Extra Items</Text>}
                            <VStack space={5} px={2}>
                                {TicketStore.orderTicket.extra.map((val, index) => {
                                    return (
                                        <HStack justifyContent={'space-between'} alignItems={'center'}>
                                            ...
                                            <HStack>
                                                <Text fontSize={18} fontWeight={'500'}
                                                      shadow={1}>{val.title} x {val.quantity}</Text>
                                            </HStack>
                                        </HStack>
                                    )
                                })}
